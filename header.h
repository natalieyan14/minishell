...
/*   Updated: 2025/11/25 13:44:57 by natalieyan       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int							handle_word_token(t_command *cmd, t_token *token,
								int *idx);
int							join_prev_if_no_space(t_command *cmd,
								t_token *tokens, int pos, int *idx);
char						*get_merged_start(t_token *tokens, int start);
char						*merge_adjacent_words(t_token *tokens, int start,
								int end, int *next_out);
int							handle_redirection(t_command *cmd, t_token *tokens,
								int start, int end);
int							process_redir_token(t_command *cmd, t_token *tokens,
								int *start, int end);
int							process_word(t_command *cmd, t_token *tokens,
								int *start, int *j);
int							process_token(t_command *cmd, t_token *tokens,
								t_process_data *data);
